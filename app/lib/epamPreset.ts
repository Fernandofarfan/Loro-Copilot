...
Senior Python Backend Engineer and Technical Lead with 8+ years of experience designing scalable microservices architectures, high-throughput RESTful APIs, and enterprise integration backends. Specialist in Python 3.10+ (FastAPI, Asyncio, uvloop, Celery, SQLAlchemy 2.0, Pydantic v2), optimized data persistence in PostgreSQL (with pgvector for semantic search and AI/LLM integrations) and Redis, and containerized cloud workloads on AWS and Google Cloud Platform.
Strong track record leading the adoption of Clean Architecture, Domain-Driven Design (DDD), automated testing suites with Pytest under TDD methodologies, and distributed observability with OpenTelemetry / IBM Instana. Bridges business objectives and technical scalability with strategic team leadership, backed by a background in Information Systems and advanced coursework in Labor Relations (UBA).
...
- Python Frameworks & Ecosystem: FastAPI, Pydantic v2, SQLAlchemy 2.0 (Async Engine & ORM), GenAI / LLM Integration (LangChain, Vertex AI, OpenAI API, RAG, pgvector), Alembic (Zero-Downtime Migrations), Celery, Django / DRF, Flask, httpx, Uvicorn, Gunicorn, Streaming APIs (SSE / WebSockets).
- Databases, Caching & Messaging: PostgreSQL (Advanced Modeling, pgvector, Composite Indexes, JSONB, asyncpg, PgBouncer), Redis (Distributed Caching, Rate Limiting, Pub-Sub, Celery Broker), MySQL, MongoDB, AWS SQS/SNS, Google Cloud Pub/Sub.
- Cloud & DevOps: AWS (ECS, EKS, Lambda, S3, SQS, RDS, ElastiCache), Google Cloud Platform (Cloud Run, GKE, Cloud Storage, Secret Manager), Docker (Multi-stage builds), Docker Compose, Kubernetes, Terraform, GitHub Actions CI/CD.
...
- Technical Lead & Senior Systems Engineer | Reforest Latam (May 2026 – Present | Full-time, Remote)
  * Led adoption of Clean Architecture & DDD across 6 engineers, formalizing ADRs and accelerating feature delivery by 35%.
  * Established >85% automated test coverage across microservices with Pytest under TDD.
  ...
  * Engineered cloud-native security layers connecting microservices to cloud resources under JWT/OAuth2 and least-privilege principles.
  * Automated CI/CD workflows in GitHub Actions for zero-downtime container deployments.

- Senior Python & Software Consultant | Independent Consulting & Enterprise Projects (Jan 2021 – Present | Remote)
  * Eliminated N+1 query bottlenecks and optimized async persistence with SQLAlchemy 2.0, Pydantic v2, and Redis caching (>60% reduction in p95 latency).
  ...
  * Designed high-concurrency REST APIs in FastAPI with strict schema validation, async streaming, and structured exception handling.
  * Standardized reproducible environments via multi-stage Docker builds and Terraform.
...
- Senior Software Engineer & Database Administrator | University of Buenos Aires (UBA) (March 2018 – Feb 2024)
  ...
  * Implemented database schema versioning with Alembic and automated testing on PostgreSQL and SQL Server.
  * Optimized complex SQL queries and stored procedures across high-demand transactional databases.
...
- Systems Infrastructure & Operations Coordinator | Government of the City of Buenos Aires (GCBA) (Jan 2020 – Dec 2023)
  ...
  * Maintained 99.9% operational uptime SLA across municipal platforms.

EDUCATION & CERTIFICATIONS
- Associate Degree in Information Systems | UTN (Expected Dec 2027)
- Labor Relations & HR | UBA (Advanced Coursework in Organizational Leadership & Team Dynamics)
- Meta Back-End Developer | IBM Java Developer | IBM JS Backend | IBM DevOps & Software Engineering | Meta Full-Stack
- Languages: Spanish (Native), English (B2 / Professional Working Proficiency)`;

export const EPAM_MARKDOWN_ANSWERS = `# Informe de Entrevista — EPAM (Puesto: Python Backend Engineer & Technical Lead)
...
export function getEpamMasterAnswers(): MasterAnswer[] {
  return parseInterviewMarkdownToMasterAnswers(
    EPAM_MARKDOWN_ANSWERS,
    EPAM_PRESET_COMPANY,
    EPAM_PRESET_ROLE
  );
}
